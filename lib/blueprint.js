...
//	 │
// 	 ├─ Private fields
// 	 │   ├─ dungeon		← MEIRO.Dungeon
// 	 │   ├─ image		← THREE.Group, image of the dungeon floor map
...
//	 │   ├─ addFrame( )
//	 │   ├─ addFloorTiles( )
//	 ├─ walls[] 	← addWalls( dungeon )
//	 ├─ thinWalls[]	← addThinWalls( dungeon )
//	 ├─ stairs[]	← addStairs( dungeon )
//	 ├─ models[]	← addModels( dungeon )
//	 └─ players[]	← addPlayers( )
//
...
MEIRO.Blueprint = class
{
	constructor( dungeon )
	{
		...
		// a group of all visual elements of the floor plan
		...
		this.addFrame();
		this.addFloorTiles( );
		this.addWalls( dungeon );
		this.addThinWalls( dungeon );
		this.addStairs( dungeon );
		this.addModels( dungeon );
		this.addPlayers( );
	...
	// add a frame for the whole dungeon
	addFrame()
	{
	...
	addFloorTiles( )
	{
		...
		this.image.floorTiles = new THREE.Group();
		this.image.add( this.image.floorTiles );
		...
		for( var level of this.dungeon.levels )
		{
			...
			this.image.floorTiles.add( mesh );
			
		} // for level of dungeon.levels
			
	} // MEIRO.Blueprint.addFloors
	...
	addThinWalls( dungeon )
	{
		...
		for( var level of dungeon.levels )
		{
			...
			var geometry  = new THREE.BufferGeometry().setFromPoints( points ),
				...
				mesh.position.y = level.levelIndex * MEIRO.STAIRS.HEIGHT;
				mesh.renderOrder  = 10000 + 100*level.levelIndex + 3;
				
			this.thinWalls.push( mesh );
			this.image.add( mesh );
			
		} // for level of dungeon.levels

	} // MEIRO.Blueprint.addThinWalls
	...
	} // MEIRO.Blueprint.addModels
	
	

	// blueprint image of the players
	addPlayers( )
	{
	...
	update( )
	{
		...
		if( options.lowpoly )
		{
			for( var i=0; i<n; i++ )
			{
				...
				this.floors[i].visible = visible;
				this.models[i].visible = visible;
				this.thinWalls[i].visible = visible;
				this.walls[i].visible = visible;
				
				visible = visible || ( (i-1) == this.targetLevel);
				this.stairs[i].visible = visible;
			}
		}
		else
		{
			...
			for( var i=0; i<n; i++ )
			{
				...
				this.image.floorTiles.children[i].material.opacity = 1.0*opacity + 0.1*subOpacity;
				this.thinWalls[i].material.opacity  = 0.9*opacity + 0.5*subOpacity;
				this.models[i].material.opacity = 1.0*opacity + 0.5*subOpacity;
				...
				this.stairs[i].material.opacity = 2*opacity + 0.2*subOpacity;
			}
		}
