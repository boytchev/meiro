...
MEIRO.Blueprint = class
{
	...
	update( )
	{
		...
		else
		{
			...
			var k = Math.pow( Math.cos(controls.rot.y), 5 );

			function opacityGradient( opacities, level )
			{
				level = THREE.Math.clamp( Math.abs( level ), 0, opacities.length-1 );
				
				var fromIndex = Math.floor( level ),
					toIndex = Math.round( level );
					
				var opacity = fromIndex==toIndex ? opacities[fromIndex] : THREE.Math.mapLinear( level, fromIndex, toIndex, opacities[fromIndex], opacities[toIndex] ); 

				if( fromIndex ) opacity *= k;
			...
			for( var i=0; i<n; i++ )
			{
				this.imageFloors[i].material.opacity = opacityGradient( [0.9, 0.1, 0], this.showLevel-i );
				this.imageThinWalls[i].material.opacity = opacityGradient( [1, 0.5, 0.3, 0.1, 0], this.showLevel-i );
				this.imageModels[i].material.opacity = opacityGradient( [1, 0.5, 0.1], this.showLevel-i );
				this.imageWalls[i].material.opacity  = opacityGradient( [1, 0], this.showLevel-i );
				this.imageStairs[i].material.opacity  = opacityGradient( [1.5, 0.05, 0], this.showLevel-i+0.5 );
			}
		}
	...
	goLevelUp()
	{
		...
		var that = this;
		new TWEEN.Tween({level:that.showLevel}).to({level:that.targetLevel},MEIRO.BLUEPRINT.SPEED)
