...
MEIRO.DungeonLevel = class
{
	...
	nextAttempt:
		for (; attempts; attempts--)
		{
			...
			{
				if( min.x>max.x ) {var p = min.x; min.x = max.x; max.x = p;}
				if( min.z>max.z ) {var p = min.z; min.z = max.z; max.z = p;}
			...
			function addFloorToFloor(min,max)
			{
				if( min.x>max.x ) {var p = min.x; min.x = max.x; max.x = p;}
				if( min.z>max.z ) {var p = min.z; min.z = max.z; max.z = p;}
				that.floors.push( {min:min, max:max} );
			...
			//cutHoleInFloor({x:midUp.x-0.5,z:midUp.z-0.5},{x:midUp.x+0.5,z:midUp.z+0.5});
			cutHoleInFloor(
				{x:midUp.x+0.5*vec.x+0.5*nor.x,z:midUp.z+0.5*vec.z+0.5*nor.z},
				{x:midUp.x-0.5*vec.x-0.5*nor.x,z:midUp.z-0.5*vec.z-0.5*nor.z}
			);
			cutHoleInFloor(
				{x:midUp.x+1.5*vec.x+0.5*nor.x,z:midUp.z+1.5*vec.z+0.5*nor.z},
				{x:midUp.x+0.5*vec.x-0.5*nor.x,z:midUp.z+0.5*vec.z-0.5*nor.z}
			);
			
			// add thin floors to the left and right side of the hole
			// this prevents seeing edges of the walls of lower level
			addFloorToFloor(
				{x:midUp.x+1.5*vec.x+0.5*nor.x,z:midUp.z+1.5*vec.z+0.5*nor.z},
				{x:midUp.x-0.5*vec.x+0.399*nor.x,z:midUp.z-0.5*vec.z+0.399*nor.z}
			);
			addFloorToFloor(
				{x:midUp.x+1.5*vec.x-0.5*nor.x,z:midUp.z+1.5*vec.z-0.5*nor.z},
				{x:midUp.x-0.5*vec.x-0.399*nor.x,z:midUp.z-0.5*vec.z-0.399*nor.z}
			...
			addFloorToFloor(
				{x:midUp.x+1.5*vec.x-0.399*nor.x,z:midUp.z+1.5*vec.z-0.399*nor.z},
				{x:midUp.x+1.399*vec.x+0.399*nor.x,z:midUp.z+1.399*vec.z+0.399*nor.z}
			);
//			cutHoleInFloor({x:midUp.x-0.5+vec.x,z:midUp.z-0.5+vec.z},{x:midUp.x+0.5+vec.x,z:midUp.z+0.5+vec.z});
//			cutHoleInFloor({x:midUp.x-0.4+j*vec.x,z:midUp.z-0.4+j*vec.z},{x:midUp.x+0.4+j*vec.x,z:midUp.z+0.4+j*vec.z});
