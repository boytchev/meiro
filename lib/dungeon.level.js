...
MEIRO.DungeonLevel = class
{
	...
	generateSplitSequence( sizeX, sizeZ, rooms )
	{
		...
		var sequence = split( sizeX, sizeZ, rooms );
...
//		this.rooms.push( new MEIRO.Room(min,max,this) );
		...
		var that = this;
		function split( min, max, sequence )
		{
			if( sequence == null )
			{
			...
			{
				// split along X
				var minWall = {x:min.x+sequence.pos,z:min.z};
				var maxWall = {x:min.x+sequence.pos,z:max.z};
				that.generateWall( minWall, 'z', max.z-min.z, {x:0,z:1});
				split( min, maxWall, sequence.min );
				if( !sequence.min ) sequence.min = that.rooms[that.rooms.length-1];
				split( minWall, max, sequence.max );
				if( !sequence.max ) sequence.max = that.rooms[that.rooms.length-1];
			}
			else
			{	// split along Z
				var maxWall = {x:max.x,z:min.z-sequence.pos};
				var minWall = {x:min.x,z:min.z-sequence.pos};
		...
		this.splitSequence = this.generateSplitSequence( max.x-min.x, max.z-min.z, desiredRooms );
		split( min, max, this.splitSequence );
	...
	findRoomFast(pos)
	{
		...
		function split( min, max, sequence )
		{
			if( sequence instanceof MEIRO.Room )
			{
				var room = sequence;
				if( inArea(pos,room.min,room.max) )
					return room;
			...
			//console.log(sequence.pos,min,max);
			if( sequence.pos > 0 )
			{
				// split along X
				var midX = min.x+sequence.pos;
				if( pos.x < midX )
					return split( min, {x:midX, z:max.z}, sequence.min );
				else
					return split( {x:midX, z:min.z}, max, sequence.max );
			}
			else
			{	// split along Z
				var midZ = min.z-sequence.pos;
				if( pos.z < midZ )
					return split( min, {x:max.x, z:midZ}, sequence.min );
				else
					return split( {x:min.x, z:midZ}, max, sequence.max );
			}
		}

		return split( this.dungeon.min, this.dungeon.max, this.splitSequence );

	} // MEIRO.DungeonLevel.findRoomFast
	...
	roomDoors()
	{
		for (var i =0; i<this.doors.length; i++)
		{
			...
			var room = this.findRoomFast(pos);
