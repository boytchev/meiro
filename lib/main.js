...
MEIRO.Pos = class extends THREE.Vector3
{
	constructor( x, y, z)
	{
		// if z is undefined, then y is used instead of z
		if( z===undefined )
			super( x, 0, y );
		else
			super( x, y, z );
	...
	midTo( pos )
	{
		return new MEIRO.Pos( (this.x+pos.x)/2, (this.z+pos.z)/2 );
	}
	
	shiftedBy( dX, dZ )
	{
		return new MEIRO.Pos( this.x+dX, this.z+dZ );
	}
}
...
{
	return new MEIRO.Pos( (max.x+min.x)/2, (max.z+min.z)/2 );
}
