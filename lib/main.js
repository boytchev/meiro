...
MEIRO.BACKGROUND_COLOR = 'gainsboro'; // sort of light grey
...
MEIRO.ICON = {
	// an arrow pointing up, used in blueprint to move to upper level
	UP:		{x:20, y:0, svg:'<g fill="white"><path d="M 24 68 L 12 80 L 12 26 L 0 28 L 18 0 L 36 28 L 24 26 L 24 68 "/></g>'},
	
	// an arrow pointing down, used in blueprint to move to lower level
	DOWN:	{x:20, y:20, svg:'<g fill="white"><path d="M 24 12 L 12 0 L 12 54 L 0 52 L 18 80 L 36 52 L 24 54 L 24 12 "/></g>'},
	
	// "i"-sign, used for help or additional info
	INFO:	{x:55, y:30, svg:'<g fill="white"><ellipse cx="8" cy="8" rx="8" ry="8" "/><path d="M 14 64 L 14 24 L 2 36 L 2 64 L 14 64 "/></g>'},
	
	// turn back sign, used to close the blueprint
	BACK: {x:20, y:8, svg:'<g fill="white"><path d="M 54 28 L 54 80 L 42 68 L 42 28 Q 42 24 38 24 L 26 24 L 28 36 L 0 18 L 28 0 L 26 12 L 40 12 Q 54 12 54 28"/></g>'},
}

MEIRO.STAIRS = {
	LENGTH: 3, // do not change
	HEIGHT: 2,
	STEPS: 19, // must be odd number
	STEPHEIGHT: 0.05,
	STEPDEPTH: 0.2,
};
// horizontal offset between two steps, so that all steps fit in LENGTH=3
MEIRO.STAIRS.STEP_DX = (MEIRO.STAIRS.LENGTH-MEIRO.STAIRS.STEPDEPTH) / (MEIRO.STAIRS.STEPS-1);
// vertical offset between steps, so that the bottom step lays on floor
MEIRO.STAIRS.STEP_DY = (MEIRO.STAIRS.HEIGHT-MEIRO.STAIRS.STEPHEIGHT) / (MEIRO.STAIRS.STEPS-1);
...
MEIRO.Options = function()
{
	...
	// process zoom factor
	this.zoom = paramFloat(this.urlOptions.zoom,1,3,10);

	// process language
	this.lang = this.urlOptions.lang;
...
function onWindowResize() {
	...
	if (typeof ao != 'undefined' && ao.ScreenAO)
	{
...
//		</div>
//----------------------------------------------------------
MEIRO.CornerButton = function (corner, onclick, text, icon, iconX=0, iconY=0) {
	...
	{	// it is SVG
		...
		this.icon.setAttributeNS( null, 'width', '100' );
		this.icon.setAttributeNS( null, 'height', '100' );
		this.icon.setAttributeNS( null, 'viewBox', (-icon.x||0)+' '+(-icon.y||0)+' 100 100' );
		this.icon.setAttributeNS( null, 'class', 'cornerIcon' );
		this.icon.innerHTML = icon.svg;
	}
	else
	{	// it is image
	...
	this.text.innerHTML = MEIRO.translate( text );
...
// Method for showing a corner button
MEIRO.CornerButton.prototype.showHide = function (show) {
	if( show )
		this.button.style.display = 'block';
	else
...
// Method for preparing SVG text as button icon test
function svg(text) {

	// remove text before <g> and after </g>
	text = text.split('<g>')[1].split('</g>')[0];
	
	// split by spaces
	text = text.split(' ');
	for( var i=text.length-1; i>=0; i-- )
	{
		if( text[i].indexOf('fill')==0 )
		{
			text.splice(i,1);
			continue;
		}
		
		if( text[i].indexOf('stroke')==0 )
		{
			text.splice(i,1);
			continue;
		}
		
		if( text[i].indexOf('pointer')==0 )
		{
			text[i] = '"'+text[i].split('"')[2];
			continue;
		}
		
		if( parseInt(text[i]) )
		{
			text[i] = Math.floor(parseInt(text[i])/10);
			continue;
		}
		
		if( text[i].indexOf('cx=')==0 )
		{
			text[i] = 'cx="'+Math.floor(parseInt(text[i].split('"')[1])/10)+'"';
			continue;
		}
		
		if( text[i].indexOf('cy=')==0 )
		{
			text[i] = 'cy="'+Math.floor(parseInt(text[i].split('"')[1])/10)+'"';
			continue;
		}
		
		if( text[i].indexOf('rx=')==0 )
		{
			text[i] = 'rx="'+Math.floor(parseInt(text[i].split('"')[1])/10)+'"';
			continue;
		}
		
		if( text[i].indexOf('ry=')==0 )
		{
			text[i] = 'ry="'+Math.floor(parseInt(text[i].split('"')[1])/10)+'"';
			continue;
		}
		
		if( text[i].indexOf('width=')==0 )
		{
			text[i] = 'width="'+Math.floor(parseInt(text[i].split('"')[1])/10)+'"';
			continue;
		}
		
		if( text[i].indexOf('height=')==0 )
		{
			text[i] = 'height="'+Math.floor(parseInt(text[i].split('"')[1])/10)+'"';
			continue;
		}
		
		if( text[i].indexOf('x=')==0 )
		{
			text[i] = 'x="'+Math.floor(parseInt(text[i].split('"')[1])/10)+'"';
			continue;
		}
		
		if( text[i].indexOf('y=')==0 )
		{
			text[i] = 'y="'+Math.floor(parseInt(text[i].split('"')[1])/10)+'"';
			continue;
		}
...
MEIRO.showInfo = function(element,onBeforeCloseInfo,onAfterCloseInfo)
{
	var infoPanel = document.getElementById('infoPanel');
		...
		infoPanel.onBeforeCloseInfo = onBeforeCloseInfo||null;
		infoPanel.onAfterCloseInfo = onAfterCloseInfo||null;

	var dimmer = document.getElementById('dimmer');
		dimmer.style.display = 'block';
		dimmer.addEventListener( 'mousedown', MEIRO.hideInfo, true );

	if (controls.room && controls.room.onInfo) controls.room.onInfo(element);
...
MEIRO.hideInfo = function()
{
	...
	var dimmer = document.getElementById('dimmer');
		...
		dimmer.removeEventListener( 'mousedown', MEIRO.hideInfo );
...
MEIRO.defaultLoop = function()
{
	...
	//cameraLight.target.copy(camera.target );
