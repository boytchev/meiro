...
MEIRO.Blueprint = class
{
	constructor( dungeon )
	{
		...
		var modelsCount = options.models.split(',').length;
		...
		scene.add( ...this.allFloors, ...this.allWalls, ...this.allStairs, ...this.allModels );
		
		if (options.player)
		{
			scene.add( this.imagePlayer() );
		}
	...
	showLevel( level )
	{
		...
		for( var i=0; i<this.allStairs.length; i++ )
		{
			var dist = Math.abs(level-i);
			if( level<i ) dist--;
