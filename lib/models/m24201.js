...
//	Модел 24201 - Повърхност на Безие - плавно оцветяване и осветяване
//	П. Бойчев, 2017



// конструктор на модела
MEIRO.Models.M24201 = function M24201(room)
{
	...
	this.m = options.lowpoly?10:24; // точки по един контур
	this.bezierGrid = new THREE.ParametricBufferGeometry(function(u,v){return new THREE.Vector3(0,0,0)},this.m-1,this.m-1);
	this.bezierObject = new THREE.Mesh( this.bezierGrid, new THREE.MeshPhongMaterial({color:'cornflowerblue',side: THREE.DoubleSide,shininess:200}) );
...
MEIRO.Models.M24201.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M24201.DIST = {MIN:10, MAX:30, HEIGHT:0};
MEIRO.Models.M24201.POS = {DIST:20, ROT_X:-1, ROT_Y:0.6};
MEIRO.Models.M24201.ROT_Y = {MIN:-0.1, MAX:0.7};

MEIRO.Models.M24201.prototype.bezierSum = function(u,v)
{
	var B = [];
...
MEIRO.Models.M24201.prototype.bezierCurve = function(t,p0,p1,p2,p3)
{
...
// аниматор на модела
MEIRO.Models.M24201.prototype.onAnimate = function(time)
{
	var k = 0;
	...
	this.bezierObject.geometry.computeVertexNormals();
...
MEIRO.Models.M24201.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Повърхност на Безие - плавно оцветяване и осветяване</h1>';

	s += '<p>За постигане на плавно оцветяване и осветяване се генерира нормален вектор не за всяка стена, а за всеки връх от мрежата на повърхността на Безие. Този вектор се получава като векторно произведение на u и v страната на триъгълник, който има този връх.';
...
MEIRO.Models.M24201.prototype.onToggle = function(element)
{
	var that = this;
