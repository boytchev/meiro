...
//	Модел 24181 - Повърхност на Безие - оцветяване и осветяване
//	П. Бойчев, 2017



// конструктор на модела
MEIRO.Models.M24181 = function M24181(room)
{
	...
	this.m = options.lowpoly?10:16; // точки по един контур
	this.bezierGrid = new THREE.ParametricBufferGeometry(function(u,v){return new THREE.Vector3(0,0,0)},this.m-1,this.m-1);
	this.bezierObject = new THREE.Mesh( this.bezierGrid, new THREE.MeshPhongMaterial({color:'cornflowerblue',side: THREE.DoubleSide, shading:THREE.FlatShading,shininess:100}) );
	...
	this.light = new THREE.PointLight('white',1/2);
	this.light.position.set(0,5,0);
	
	// сглобяване на целия модел
	this.image.add(this.object,this.bezierObject,this.light);
}

MEIRO.Models.M24181.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M24181.DIST = {MIN:10, MAX:30, HEIGHT:0};
MEIRO.Models.M24181.POS = {DIST:20, ROT_X:-1, ROT_Y:0.6};
MEIRO.Models.M24181.ROT_Y = {MIN:-0.1, MAX:0.7};

MEIRO.Models.M24181.prototype.bezierSum = function(u,v)
{
	var B = [];
...
MEIRO.Models.M24181.prototype.bezierCurve = function(t,p0,p1,p2,p3)
{
...
// аниматор на модела
MEIRO.Models.M24181.prototype.onAnimate = function(time)
{
	var k = 0;
	...
	this.bezierObject.geometry.computeFaceNormals();
...
MEIRO.Models.M24181.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Повърхност на Безие - оцветяване и осветяване</h1>';

	s += '<p>Повърхностната мрежа се представя като триъгълници. Двете страни на всеки от тях са по двете направления на повърхността на Безие. Векторите по тези страни, умножени векторно, определят нормалния вектор на триъгълника, чрез който се получава правилното оцветяване и осветяване.';
...
MEIRO.Models.M24181.prototype.onToggle = function(element)
{
	var that = this;
