...
//	Модел 24151 - Повърхност на Безие - връхни точки
//	П. Бойчев, 2017



// конструктор на модела
MEIRO.Models.M24151 = function M24151(room)
{
	...
	for (var i=0; i<4; i++)
	{
		...
		for (var j=0; j<4; j++)
		{
			...
			this.points[i][j].scale.set(edge?0.3:0.1,edge?0.3:0.1,edge?0.3:0.1);
			if (!edge) this.points[i][j].material = MEIRO.PRIMITIVE.STYLE.PAWN;
...
MEIRO.Models.M24151.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M24151.DIST = {MIN:10, MAX:30, HEIGHT:0};
MEIRO.Models.M24151.POS = {DIST:20, ROT_X:-1, ROT_Y:0.6};
MEIRO.Models.M24151.ROT_Y = {MIN:-0.1, MAX:0.7};

// аниматор на модела
MEIRO.Models.M24151.prototype.onAnimate = function(time)
{
	var k = 0;
	for (var i=0; i<4; i++)
		for (var j=0; j<4; j++)
...
MEIRO.Models.M24151.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Повърхност на Безие - връхни точки</h1>';

	s += '<p>Връхните точки на повърхност на Безие съответстват на точките с uv-координати (0,0), (0,1), (1,0) и (1,1). Тези точки съвпадат с връхните четири точки от мрежата от контролни точки.';
...
MEIRO.Models.M24151.prototype.onToggle = function(element)
{
	var that = this;
