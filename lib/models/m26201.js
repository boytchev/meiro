...
//	Модел 26201 - Анимация на слоеве чрез ротация
//	П. Бойчев, 2017


// конструктор на модела
MEIRO.Models.M26201 = function M26201(room)
{
	...
	for (var i=0; i<10; i++)
	{
		...
		p.position.set(20*Math.random()-10,Math.random()/10-3,20*Math.random()-10);
		this.pagoda.push( p );
	}
...
MEIRO.Models.M26201.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M26201.DIST = {MIN:10, MAX:30, HEIGHT:0};
MEIRO.Models.M26201.POS = {DIST:15, ROT_X:0, ROT_Y:0};
MEIRO.Models.M26201.ROT_Y = {MIN:-0.1, MAX:0.7};


// аниматор на модела
MEIRO.Models.M26201.prototype.onAnimate = function(time)
{
	for (var i=0; i<10; i++)
	{
		...
		{
			var s = Math.sin(0.001*time+offset);
			p.rotation.x = s*this.k/2;
			p.rotation.y = s*(1-this.k);
		}
	}
...
MEIRO.Models.M26201.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Анимация на слоеве чрез ротация</h1>';

	s += '<p>Промяната на ъгълът между два съседни слоя може да генерира две различни движения. Ако ротацията е около вертикалната ос, движението е като усукване около нея. Ако ротацията е около напречна ос, движението е навиване на спирала.</p>';
...
MEIRO.Models.M26201.prototype.onToggle = function(element)
{
	var that = this;
