...
//	Модел 07581 - Равномерно разпределени цветове
//	П. Бойчев, 2017


// конструктор на модела
MEIRO.Models.M07581 = function M07581(room)
{
	...
	var that = this;
	this.toggle = new MEIRO.CornerButton('topLeft', function(){that.onToggle();}, '', 'images/n234.png');
	this.toggle.state = -1;
	this.toggle.stateTexts = ['Два цвята','Три цвята','Четири цвята'];
...
MEIRO.Models.M07581.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M07581.DIST = {MIN:10, MAX:20, HEIGHT:0};
MEIRO.Models.M07581.POS = {DIST:13, ROT_X:0, ROT_Y:0};
MEIRO.Models.M07581.ROT_Y = {MIN:-0.1, MAX:0.7};


MEIRO.Models.M07581.prototype.setPositionYZ = function(index,y,z)
{
...
MEIRO.Models.M07581.prototype.setPositionRA = function(index,r,a)
{
	var ring = this.rings[index];
...
MEIRO.Models.M07581.prototype.onObjectPos = function()
{
	// координати на мишка
...
MEIRO.Models.M07581.prototype.onObject = function()
{
	// координати на мишка
	this.mouse.x = (controls.human.mousePos.x/window.innerWidth)*2 - 1;
	this.mouse.y = -(controls.human.mousePos.y/window.innerHeight)*2 + 1;
...
MEIRO.Models.M07581.prototype.onDragMove = function()
{
	...
	if (p)
	{
		...
		for (var i=0; i<this.m; i++)
		{
			...
			if (i!=this.dragIndex)
				this.setPositionRA(i,q.distance,r.angle+(i-this.dragIndex)*2*Math.PI/this.m);
...
// аниматор на модела
MEIRO.Models.M07581.prototype.onAnimate = function(time)
{
//	TWEEN.update();
//	reanimate();
}
...
MEIRO.Models.M07581.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Равномерно разпределени цветове</h1>';

	s += '<p>При избор на равномерно разпределени цветове, равномерността е относно ориентацията на цветовете. При два цвята, те са срещуположно един на друг. При три цвята са през 120°, а при четири &ndash; 240°.</p>';
	s += '<p>Наситеността на цветовете, това е разстоянието им до центъра, може да се променя независимо от ориентацията им.</p>';
	element.innerHTML = s;
}
...
MEIRO.Models.M07581.prototype.onToggle = function(element)
{
	this.toggle.state = (this.toggle.state+1)%3;
	this.toggle.setText(this.toggle.stateTexts[this.toggle.state]);
	this.m = this.toggle.state+2;
	...
	{
		var q = this.rings[i];
		this.setPositionRA(i,q.distance,r.angle+i*2*Math.PI/this.m);
		this.lines[i].setTo(new THREE.Vector3(0.11,(q.distance-0.2)*Math.cos(q.angle),(q.distance-0.2)*Math.sin(q.angle)));
		this.blocks[i].material.color = this.calcWheelColor(q.distance*Math.cos(q.angle)/4,q.distance*Math.sin(q.angle)/4);
	}
...
MEIRO.Models.M07581.prototype.calcWheelColor = function (x,z)
{
	function coss(x)
	{
		x = (x+360)%360;
...
MEIRO.Models.M07581.prototype.colorizeWheel = function (object)
{
