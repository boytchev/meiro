...
//	Модел 26191 - Анимация на слоеве чрез динамично мащабиране
//	П. Бойчев, 2017


// конструктор на модела
MEIRO.Models.M26191 = function M26191(room)
{
...
MEIRO.Models.M26191.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M26191.DIST = {MIN:10, MAX:30, HEIGHT:0};
MEIRO.Models.M26191.POS = {DIST:15, ROT_X:0, ROT_Y:0};
MEIRO.Models.M26191.ROT_Y = {MIN:-0.1, MAX:0.7};


// аниматор на модела
MEIRO.Models.M26191.prototype.onAnimate = function(time)
{
	for (var i=0; i<15; i++)
	{
		...
		{
			var s = 0.9+0.4*Math.sin(0.005*time-0.5*n+offset);
			p.scale.set(s,0.85,s);
		}
	}
	reanimate();
}
...
MEIRO.Models.M26191.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Анимация на слоеве чрез динамично мащабиране</h1>';

	s += '<p>Мащабът между два съседни слоя може да не е константа, а да се променя както с времето, така и с номера на слоя.</p>';
	element.innerHTML = s;
}
