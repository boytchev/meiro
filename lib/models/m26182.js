...
//	Модел 26182 - Анимация на слоеве чрез мащабиране
//	П. Бойчев, 2017


// конструктор на модела
MEIRO.Models.M26182 = function M26182(room)
{
...
MEIRO.Models.M26182.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M26182.DIST = {MIN:10, MAX:30, HEIGHT:0};
MEIRO.Models.M26182.POS = {DIST:15, ROT_X:0, ROT_Y:0};
MEIRO.Models.M26182.ROT_Y = {MIN:-0.1, MAX:0.7};


// аниматор на модела
MEIRO.Models.M26182.prototype.onAnimate = function(time)
{
	for (var i=0; i<15; i++)
	{
		var offset = Math.sin(i);
		var s = 0.75+0.25*Math.sin(rpm(time,30)+offset);
...
MEIRO.Models.M26182.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Анимация на слоеве чрез мащабиране</h1>';

	s += '<p>Анимацията в този модел е постигната чрез мащабиране между всеки два слоя. Това, което се променя, е размерът на локалната координатна система на един слой, спрямо локалната координатна система на предходния слой.</p>';
	element.innerHTML = s;
}
