...
//	Модел 26212 - Модел на камшик
//	П. Бойчев, 2017


// конструктор на модела
MEIRO.Models.M26212 = function M26212(room)
{
	...
	function Pagoda(n)
	{
		...
		for (var p=pagoda; n>1; n--)
		{
			var newP = Floor();
			newP.position.y = 5;
			newP.scale.set(0.875,0.875,0.875);
			p.add( newP );
			p.subpagoda = newP;
			p = newP;
		}
	...
	for (var i=0; i<10; i++)
	{
		var p = Pagoda(10+20*Math.random());
		p.position.set(16*Math.random()-8,Math.random()/10-3,16*Math.random()-8);
		this.pagoda.push( p );
	}
...
MEIRO.Models.M26212.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M26212.DIST = {MIN:10, MAX:30, HEIGHT:0};
MEIRO.Models.M26212.POS = {DIST:15, ROT_X:0, ROT_Y:0};
MEIRO.Models.M26212.ROT_Y = {MIN:-0.1, MAX:0.7};


// аниматор на модела
MEIRO.Models.M26212.prototype.onAnimate = function(time)
{
	for (var i=0; i<10; i++)
	{
		var offset = Math.sin(i);
		for (var p=this.pagoda[i].subpagoda,n=0; p; p=p.subpagoda,n++)
		{
			p.rotation.x = 0.5*Math.sin(0.0013*time+n/4+offset)*Math.pow(0.8,n);
			p.rotation.y = 0.5*Math.cos(0.0011*time+n/6-offset)*Math.pow(0.9,n);
		}
	}
...
MEIRO.Models.M26212.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Модел на камшик</h1>';

	s += '<p>Моделирането на камшик е подобно на извиването на пипало. Основната разлика е в намаляване на ъгъла на въртене с преминаване към по-крайните слоеве. По този начин огъването на конструкцията е най-видимо в началната и средната ѝ част, а върхът е "втвърден" и не се огъва.</p>';
