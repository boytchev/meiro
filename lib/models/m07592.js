...
//	Модел 07592 - Съседни цветове
//	П. Бойчев, 2017


// конструктор на модела
MEIRO.Models.M07592 = function M07592(room)
{
	...
	for (var i=0; i<this.n; i++)
	{
		...
		this.setPositionRA(i,2,(i-0.5)/this.m*4*Math.PI/3);
	}
...
MEIRO.Models.M07592.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M07592.DIST = {MIN:10, MAX:20, HEIGHT:0};
MEIRO.Models.M07592.POS = {DIST:13, ROT_X:0, ROT_Y:0};
MEIRO.Models.M07592.ROT_Y = {MIN:-0.1, MAX:0.7};


MEIRO.Models.M07592.prototype.setPositionYZ = function(index,y,z)
{
...
MEIRO.Models.M07592.prototype.setPositionRA = function(index,r,a)
{
	var ring = this.rings[index];
...
MEIRO.Models.M07592.prototype.onObjectPos = function()
{
	// координати на мишка
...
MEIRO.Models.M07592.prototype.onObject = function()
{
	// координати на мишка
	this.mouse.x = (controls.human.mousePos.x/window.innerWidth)*2 - 1;
	this.mouse.y = -(controls.human.mousePos.y/window.innerHeight)*2 + 1;
...
MEIRO.Models.M07592.prototype.onDragMove = function()
{
	...
	if (p)
	{
		...
		var span = this.rings[this.m-1].angle-this.rings[0].angle;
		span = (span%(2*Math.PI)+2*Math.PI)%(2*Math.PI);
		
		for (var i=0; i<this.m; i++)
		{
			var q = this.rings[i];
			var a = this.rings[0].angle+i/(this.m-1)*span;
...
// аниматор на модела
MEIRO.Models.M07592.prototype.onAnimate = function(time)
{
//	TWEEN.update();
//	reanimate();
}
...
MEIRO.Models.M07592.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Съседни цветове</h1>';

	s += '<p>При избор на съседни цветове те са разположени равномерно, но не из целия спектър. Така се формира комплект от цветове с една и съща гама.</p>';
	element.innerHTML = s;
}
...
MEIRO.Models.M07592.prototype.onToggle = function(element)
{
	var span = this.rings[this.m-1].angle-this.rings[0].angle;
	span = (span%(2*Math.PI)+2*Math.PI)%(2*Math.PI);
...
MEIRO.Models.M07592.prototype.calcWheelColor = function (x,z)
{
	function coss(x)
	{
		x = (x+360)%360;
...
MEIRO.Models.M07592.prototype.colorizeWheel = function (object)
{
