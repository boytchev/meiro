
//	Основи на Компютърната Графика
//	Модел 21411 - Изглаждане при текстури
//	П. Бойчев, 2017



// конструктор на модела
MEIRO.Models.M21411 = function M21411(room)
{
	MEIRO.Model.apply(this, arguments);

	var data= 'data:image/png;base64,';

	this.mapIndex = 0;

	this.maps= [];
	
	var map = new THREE.TextureLoader().load(data);
	map.wrapS = THREE.RepeatWrapping;
	map.wrapT = THREE.RepeatWrapping;
	map.repeat.set( 2, 2 );
	map.magFilter = THREE.NearestFilter;
	map.minFilter = THREE.NearestFilter;
	map.anisotropy = 1;
	this.maps.push(map);
	
	var map = new THREE.TextureLoader().load(data);
	map.wrapS = THREE.RepeatWrapping;
	map.wrapT = THREE.RepeatWrapping;
	map.repeat.set( 2, 2 );
	map.magFilter = THREE.LinearFilter;
	map.minFilter = THREE.LinearMipMapLinearFilter;
	map.anisotropy = 4;
	this.maps.push(map);
	
	// плочка
	this.plate = new MEIRO.Cube(1);
	this.plate.scale.set(10,0.1,10);
	this.plate.material = new THREE.MeshBasicMaterial( {map:this.maps[this.mapIndex]} );
	
	// бутон за превключване
	var that = this;
	this.toggle = new MEIRO.CornerButton('topLeft', function(){that.onToggle();}, 'ИЗГЛАЖДАНЕ', 'images/show.hide.png');
	this.toggle.hide();
	
	// сглобяване на целия модел
	this.image.add(this.plate);
}

MEIRO.Models.M21411.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M21411.DIST = {MIN:4, MAX:16, HEIGHT:0};
MEIRO.Models.M21411.POS = {DIST:7, ROT_X:1.57, ROT_Y:0.2};
MEIRO.Models.M21411.ROT_Y = {MIN:-0.1, MAX:0.7};


// аниматор на модела
MEIRO.Models.M21411.prototype.onAnimate = function(time)
{
	//reanimate();
}



// информатор на модела
MEIRO.Models.M21411.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Изглаждане при текстури</h1>';

	s += '<p>Този модел демонстрира два от вариантите на показване на текстура &ndash; без заглаждане и със заглаждане. Разликата между двата варианта е най-видима в най-близката и най-далечната част от текстурата.</p>';
	
	element.innerHTML = s;
}


// превключвател на модела
MEIRO.Models.M21411.prototype.onToggle = function(element)
{	
	this.mapIndex = 1-this.mapIndex;
	this.plate.material.map = this.maps[this.mapIndex]; 

	reanimate();
}
	