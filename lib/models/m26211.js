...
//	Модел 26211 - Модел на пипало
//	П. Бойчев, 2017


// конструктор на модела
MEIRO.Models.M26211 = function M26211(room)
{
...
MEIRO.Models.M26211.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M26211.DIST = {MIN:10, MAX:30, HEIGHT:0};
MEIRO.Models.M26211.POS = {DIST:15, ROT_X:0, ROT_Y:0};
MEIRO.Models.M26211.ROT_Y = {MIN:-0.1, MAX:0.7};


// аниматор на модела
MEIRO.Models.M26211.prototype.onAnimate = function(time)
{
	for (var i=0; i<10; i++)
	{
		...
		{
			p.rotation.x = 0.5*Math.sin(0.0013*time+n/4+offset);
			p.rotation.y = 0.5*Math.cos(0.0011*time+n/6-offset);
		}
	}
...
MEIRO.Models.M26211.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Модел на пипало</h1>';

	s += '<p>Гърченето и извиването на пипало може да се реализира чрез слоеве и едновременна ротация както по надлъжната ос, така и поне по една от напречните оси. В този смисъл, извиването е комбинация от две движения &ndash; усукване и навиване на спирала.</p>';
