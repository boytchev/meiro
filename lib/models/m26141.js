...
//	Модел 26141 - Пагода чрез слоеве
//	П. Бойчев, 2017


// конструктор на модела
MEIRO.Models.M26141 = function M26141(room)
{
	...
	function Floor()
	{
		var room = new THREE.Mesh(
			new THREE.CylinderGeometry(2*Math.SQRT2,2*Math.SQRT2,4,4,1,true),
			new THREE.MeshBasicMaterial({map:map})
		);
		room.position.y = 1;
		
		var roof = new THREE.Mesh(
			new THREE.CylinderGeometry(4*Math.SQRT2,2*Math.SQRT2,2,4,1),
	...
	for (var i=0; i<20; i++)
	{
		var p = Pagoda(5+10*Math.random());
		p.position.set(20*Math.random()-10,Math.random()-8,20*Math.random()-10);
		this.pagoda.push( p );
	}
...
MEIRO.Models.M26141.prototype = Object.create(MEIRO.Model.prototype);

MEIRO.Models.M26141.DIST = {MIN:10, MAX:30, HEIGHT:0};
MEIRO.Models.M26141.POS = {DIST:15, ROT_X:0, ROT_Y:0};
MEIRO.Models.M26141.ROT_Y = {MIN:-0.1, MAX:0.7};


// аниматор на модела
MEIRO.Models.M26141.prototype.onAnimate = function(time)
{
...
MEIRO.Models.M26141.prototype.onInfo = function(element)
{
	var s = '';
	s += '<h1>Пагода чрез слоеве</h1>';

	s += '<p>Всяка пагода е конструирана чрез вложени един над друг еднакви етажи. Вложеността е в смисъл, че всеки етаж е като подчинен елемент на етажа под него.</p>';
	element.innerHTML = s;
}
