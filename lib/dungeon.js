...
MEIRO.initDungeonRoom = function()
{
	...
	MEIRO.GEOMETRY = {
			...
			GLASS:	new THREE.BoxBufferGeometry(0.9,0.33,0.004),
			POLE: function()
				{
...
//					opacity: 0.4,
	...
	if (options.textures)
	{
		...
		MEIRO.STYLE.WALL.bumpMap = MEIRO.proceduralWallBumpTexture(MEIRO.TEXTURE_SIZE/4);
		MEIRO.STYLE.WALL.specularMap = MEIRO.proceduralWallSpecularTexture(MEIRO.TEXTURE_SIZE/8);
		
		MEIRO.STYLE.PILLAR.map = MEIRO.proceduralPillarTexture(MEIRO.TEXTURE_SIZE);
		MEIRO.STYLE.PILLAR.bumpMap = MEIRO.proceduralPillarBumpTexture(MEIRO.TEXTURE_SIZE/4);
		MEIRO.STYLE.PILLAR.specularMap = MEIRO.proceduralPillarSpecularTexture(MEIRO.TEXTURE_SIZE/8);
		
		MEIRO.STYLE.PILLAR4.map = MEIRO.proceduralPillar4Texture(MEIRO.TEXTURE_SIZE);
		MEIRO.STYLE.PILLAR4.bumpMap = MEIRO.proceduralPillar4BumpTexture(MEIRO.TEXTURE_SIZE/4);
		MEIRO.STYLE.PILLAR4.specularMap = MEIRO.proceduralPillar4SpecularTexture(MEIRO.TEXTURE_SIZE/8);
