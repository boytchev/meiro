...
MEIRO.HumanInteraction.prototype.touchCenter = function()
{
	...
	return new THREE.Vector2(this.touchCen.x/size.x*2-1,1-2*this.touchCen.y/size.y);
}
...
MEIRO.OrbitControls = function()
{
	...
	// panning position (angles)
	this.pan = new THREE.Vector2(0,0);
	this.panDelta = new THREE.Vector2(0,0);//0.05,0.1);
	this.panTarget = new THREE.Vector2(0,0.5);
...
MEIRO.OrbitControls.prototype.update = function()
{
	...
	if (this.human.keyShift())
	{
		...
		if (this.human.keyLeft()) {this.panDelta.x = -5/fps; this.startUp = false;}
		if (this.human.keyRight()) {this.panDelta.x = +5/fps; this.startUp = false;}

		// pan up-down with shift+keys --> no vertical panning, it could be confused with zooming
		//if (this.human.keyUp()) {this.panDelta.y = -5/fps; this.startUp = false;}
		//if (this.human.keyDown()) {this.panDelta.y = +5/fps; this.startUp = false;}
	}
	else
	{
		// pan left-right with home/end
		if (this.human.keyHome()) {this.panDelta.x = -5/fps; this.startUp = false;}
		if (this.human.keyEnd()) {this.panDelta.x = +5/fps; this.startUp = false;}
	...
	{
		this.panDelta.x = mouseMove.x/(1.75*fps); //add(mouseMove.divideScalar(15*fps));
	...
	var panX = this.dist/11*this.panDelta.x*Math.sin(this.rot.x) + this.dist/11*this.panDelta.y*Math.cos(this.rot.x);
	var panY = -this.dist/11*this.panDelta.x*Math.cos(this.rot.x) + this.dist/11*this.panDelta.y*Math.sin(this.rot.x);
	this.panDelta.divideScalar(1.15);
