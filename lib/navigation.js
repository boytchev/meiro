...
MEIRO.HumanInteraction = function()
{
	...
	function onTouchStart(event)
	{
		...
		if (reanimate) reanimate(event);
		event.preventDefault();
//event.touches[1] = {clientX:event.touches[0].clientX+10, clientY:event.touches[0].clientY+10};//@@		
		...
		if (event.touches.length==2)
		{
			...
			touchPos = (event.touches[1].clientX+event.touches[0].clientX)/2;
...
//event.touches[1] = {clientX:event.touches[0].clientX+10, clientY:event.touches[0].clientY+10};//@@		
		that.keys[that.key.TOUCHONE] = (event.touches.length==1) && (animationLoop.time>that.touchTime+200);
		that.keys[that.key.TOUCHTWO] = (event.touches.length==2) && (animationLoop.time>that.touchTime+200);
...
MEIRO.FirstPersonControls.prototype.updateOrbitWalk = function()
{
	...
	var fps = THREE.Math.clamp(animationLoop.stats.fps,20,600);
...
{
	var fps = THREE.Math.clamp(animationLoop.stats.fps,10,600);
	...
	if (this.human.keyUp()) this.speed = 1.5/fps;
