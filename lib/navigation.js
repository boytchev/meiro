...
MEIRO.HumanInteraction = function()
{
	...
	this.touchFitstCen = new THREE.Vector2(0,0);
	var touchDist = 0;
	this.touchTime = 0;
	...
	function onTouchStart(event)
	{
		...
		if (event.touches.length==2)
		{
			...
			that.touchDelta = 0;
			that.touchFitstCen.set( (event.touches[1].clientX+event.touches[0].clientX)/2, (event.touches[1].clientY+event.touches[0].clientY)/2);
...
//console.log('tcen',this.human.touchCen);
	...
	{
		this.panDelta = (this.human.touchFirstCen.x-this.human.touchCen.x)/(1.75*fps);
